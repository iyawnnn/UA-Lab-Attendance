...
import { registerStudentToDatabase, getLabRooms, submitAttendance, recoverStudentDevice } from "../actions";
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-xl p-8 border border-gray-100 relative">
        <a href="/" className="absolute top-6 left-6 text-sm text-gray-400 hover:text-blue-600 transition-colors">&larr; Home</a>
        ...
        {view === "register" && (
          <div className="animate-in fade-in duration-300">
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <input type="password" placeholder="Create 4-Digit Recovery PIN" maxLength={4} pattern="\d{4}" title="Must be exactly 4 numbers" className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 outline-none" value={recoveryPin} onChange={(e) => setRecoveryPin(e.target.value)} required />
            ...
            <div className="mt-6 text-center">
              <button onClick={() => { setView("recovery"); setMessage(""); setIsError(false); }} className="text-sm text-blue-600 hover:underline font-medium">Lost your device? Recover account</button>
            </div>
          </div>
        )}
        ...
        {view === "recovery" && (
          <div className="animate-in fade-in duration-300">
            <div className="text-center mb-8 mt-4">
              <h2 className="text-2xl font-bold text-gray-800">Device Recovery</h2>
              <p className="text-gray-500 mt-2 text-sm">Enter your PIN to revoke your old device.</p>
            </div>
            <form onSubmit={handleRecovery} className="space-y-4">
              <input type="text" placeholder="Student ID" className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 outline-none" value={studentId} onChange={(e) => setStudentId(e.target.value)} required />
              <input type="password" placeholder="4-Digit Recovery PIN" maxLength={4} className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 outline-none" value={recoveryPin} onChange={(e) => setRecoveryPin(e.target.value)} required />
              
              <button type="submit" disabled={isRegistering} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded-lg mt-2">
                {isRegistering ? "Processing..." : "Revoke Old Device"}
              </button>
            </form>
            <div className="mt-6 text-center">
              <button onClick={() => { setView("register"); setMessage(""); setIsError(false); }} className="text-sm text-gray-500 hover:underline font-medium">Back to Registration</button>
            </div>
          </div>
        )}
          ...
          <div className="animate-in fade-in duration-300">
            ...
            <form onSubmit={handleLogAttendance} className="space-y-6">
              <select className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 outline-none cursor-pointer" value={selectedRoom} onChange={(e) => setSelectedRoom(e.target.value)} required>
                ...
                {labRooms.map((room, index) => <option key={index} value={room}>{room}</option>)}
              </select>
        ...
        {message && (
          <div className={`mt-6 p-4 rounded-lg text-sm font-medium border ${isError ? "bg-red-50 text-red-700 border-red-200" : (message.includes("LATE") ? "bg-yellow-50 text-yellow-800 border-yellow-300" : "bg-green-50 text-green-700 border-green-200")}`}>
            {message}
          </div>
        )}
